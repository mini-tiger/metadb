...
func (s *coreService) UpdateModelManyInstancesUnique(ctx *rest.Contexts) {

	inputData := metadata.UpdateManyModelInstanceManyUnique{}
	if err := ctx.DecodeInto(&inputData); nil != err {
		ctx.RespAutoError(err)
		return
	}
	...
	sc.IsCache()
	if !sc.Iscache {
		ctx.RespAutoError(errors.New(fmt.Sprintf("cacheMap Not Found UniqueField")))
		return
	}

	//fmt.Println(cache.CacheObjMap)
	UniqueKey := sc.Unique
