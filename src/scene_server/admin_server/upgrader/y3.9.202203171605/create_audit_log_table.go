...
func rebuildAuditLog(ctx context.Context, db dal.RDB, conf *upgrader.Config) error {

	if err := db.DropTable(ctx, common.BKTableNameAuditLog); err != nil && !mgo.IsDup(err) {
		return err
	}

	blog.Infof("start execute y3_9_202203171605")

	if err := createAuditLogTable(ctx, db, conf); err != nil {
		blog.Errorf("[upgrade y3_9_202203171605] createAuditLogTable failed, error %s", err.Error())
		return fmt.Errorf("createAuditLogTable failed, error %s", err.Error())
	}

	if err := addAuditLogTableIndex(ctx, db, conf); err != nil {
		blog.Errorf("[upgrade y3_9_202203171605] addAuditLogTableIndex failed, error %s", err.Error())
