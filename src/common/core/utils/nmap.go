...
// 业务 方法 不是通用方法
func (this *SafeMap) FindKeyAndValue(obj string, m map[string]interface{}) (err error, str string) {
	//this.RLock()
	//empty := (len(this.M) == 0)
	//this.RUnlock()
	if !this.ContainsKey(obj) {
		return errors.New(fmt.Sprintf("not found obj:%s", obj)), str
	}
	unique, _ := this.Get(obj)
	if v, ok := m[unique.(string)]; ok {
		str = v.(string)
		return
	} else {
		return errors.New(fmt.Sprintf("not found obj:%s", obj)), str
